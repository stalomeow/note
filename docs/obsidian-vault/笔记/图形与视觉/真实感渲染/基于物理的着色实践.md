...
GGX 中的粗糙度 $\alpha=\text{roughness}^2$，$\text{roughness}$ 是材质对外暴露的粗糙度参数。这是 Disney 提出的参数化方法，可以使粗糙度的变化在感知上更加线性。
...
$$
\text{atten}=\frac{\text{saturate} \left( 1-(d/r)^4 \right)^2}{d^2+1}
$$
...
$$
\text{atten}=\text{saturate} \left( \frac{\cos \theta - \cos \theta_\text{outer}}{\cos \theta_\text{inner} - \cos \theta_\text{outer}} \right)^2
$$
