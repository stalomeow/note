...
在 [[基于物理的着色理论#Normal Distribution Function|NDF 理论]] 中提到过，如果 $D(\mathbf{h})$ 是一个 NDF，那么 $D(\mathbf{h}) (\mathbf{n} \cdot \mathbf{h})$ 可以当作一个概率密度函数，其中 $\mathbf{n}$ 是表面宏观法线方向，$\mathbf{h}$ 是 half vector。因此，对于 GGX 来说，立体角 PDF 可以使用
